...
#------------------------------------------------------------------------------    
# Trajectories definition - old version of the algorithm
#------------------------------------------------------------------------------        
def compute_trajectories_old(x0,y0,z0,initial_time_index,
                             LON_nc,LAT_nc,P_nc,data,
                             list_var,list_var_advec,
                             ...
                             niter=4,BACKWARD=True):
    """ Compute Lagrangian trajectories - traj_duration is in hours - 
    this is the initial version of the algorithm, as used by Meryl"""
    
    print('Computing backward trajectories...: ')

    # Defining number od seeds 
    Number_Seeds=len(x0)
    ...
    # Time step (in seconds): temporal resolution of the trajectories
    DT=(direction*86400)/(njour*npdt)
    ...
    #North upside
    if LAT_nc[1]<LAT_nc[0]:
        reverse=True
        LAT_nc=LAT_nc[::-1] #ascending latitude
    else:
        reverse=False
    ...
    for i_ech in range(initial_time_index*npdt,npdt*initial_time_index+direction*int((npdt*njour*trajectories_duration)/24),npdt*direction):
        ...
        if reverse:
            list_var2=[]
            for i_var in [U_nc,V_nc,W_nc,U2_nc,V2_nc,W2_nc]:
                list_var2.append(i_var[:,::-1,:])
            
            [U_nc,V_nc,W_nc,U2_nc,V2_nc,W2_nc]=list_var2    
            for i_var in list_var:
                VAR_nc[i_var]=VAR_nc[i_var][:,::-1,:]
                VAR2_nc[i_var]=VAR2_nc[i_var][:,::-1,:]
        ...
        for ipd in range(0,npdt):                                              
            ...
            for ipoin in range(0,Number_Seeds):
                ...
                try:
                    u=interpn((P_nc,LAT_nc,LON_nc),U_nc,np.array([pre,la,lo]).T)
                    v=interpn((P_nc,LAT_nc,LON_nc),V_nc,np.array([pre,la,lo]).T)  
                    w=interpn((P_nc,LAT_nc,LON_nc),W_nc,np.array([pre,la,lo]).T)
                except ValueError:
                    u=np.nan
                    v=np.nan
                    w=np.nan 
                ...
                for i_var in list_var:
                    try:
                        var[i_var]=interpn((P_nc,LAT_nc,LON_nc),VAR_nc[i_var],np.array([pre,la,lo]).T)
                    except ValueError:
                        var[i_var]=np.nan
                ...
                if ipd == 0:
                    ...
                    for i_var in list_var:
                        VAR_traj[i_var][ipoin,ipdt]=var[i_var]
                ...
                if ipd > 0:
                    for i_var in list_var:
                        var[i_var]=VAR_traj[i_var][ipoin,ipdt-1]
                ...
                for iter in range(1,niter):
                    ...
                    try:
                        poistar=(ipd+1)/npdt
                        u2=interpn((P_nc,LAT_nc,LON_nc),U2_nc,np.array([pre_1,la_1,lo_1]).T)
                        v2=interpn((P_nc,LAT_nc,LON_nc),V2_nc,np.array([pre_1,la_1,lo_1]).T)
                        w2=interpn((P_nc,LAT_nc,LON_nc),W2_nc,np.array([pre_1,la_1,lo_1]).T)
                    except ValueError:
                        u2=np.nan
                        v2=np.nan
                        w2=np.nan
                    ...
                    for i_var in list_var:
                        try:
                            var2[i_var]=interpn((P_nc,LAT_nc,LON_nc),VAR2_nc[i_var],np.array([pre_1,la_1,lo_1]).T)
                        except ValueError:
                            var2[i_var]=np.nan
                ...
                varf={}
                for i_var in list_var:
                    varf[i_var]=(1-poi)*var[i_var] + poi*var2[i_var]
                ...
                LON_traj[ipoin,ipdt]=lo
                P_traj[ipoin,ipdt]=pre
                TIME_traj[ipoin,ipdt]=i_ech+(direction*ipd)
                    
                if ipd > 0:
                    U_traj[ipoin,ipdt]=uf
                    V_traj[ipoin,ipdt]=vf
                    W_traj[ipoin,ipdt]=wf
                        
                    for i_var in list_var:
                        VAR_traj[i_var][ipoin,ipdt]=varf[i_var]
