...
#------------------------------------------------------------------------------
# Computing trajectories example using ERA5 in model levels
#------------------------------------------------------------------------------
...
filename =f"ERA5_forecasts_u_v_w_temp_PV_pres_all_levels_2022-08-15a16_65N-86N.nc" 
root_input ='data/nchiab/PV/'

root_output = '/data/nchiab/PV/'
LON_nc,LAT_nc,m_nc,data = traj.read_data(filename,root_input,list_var, list_var_advect,lat='latitude',lon='longitude',pres='model_level', hPa = False)
SP_d = Dataset(f"/data/nchiab/PV/Data/ERA5_SP_2022-08-15a16_65N-86N.nc", 'r', format='NETCDF4')
SP = SP_d.variables['sp'][:]
...
def f(Sp):
  """ Function that finds the pressure depending on model level"""
  return df['ak'].to_numpy() + df['bk'].to_numpy() * Sp



LON_nc,LAT_nc,m_nc,data = traj.read_data(filename,root_input,list_var_advect, list_var,lat='latitude',lon='longitude',pres='model_level')
...
TIME_traj, LAT_traj, LON_traj, m_traj, P_traj, U_traj, V_traj, W_traj,VAR_traj=traj.compute_trajectories_ERA5(lon_seeds, lat_seeds,
                                    pres_seeds, initial_time_step, LON_nc, LAT_nc, m_nc, data, list_var, list_var_advect, SP, f,
...
traj.save_output_data_ERA5(root_output,initial_time_step,
